...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.ca = ChannelAttention(out_planes)  # 添加 ChannelAttention 模块
        self.sa = SpatialAttention()  # 添加 SpatialAttention 模块

        self.droprate = dropRate
        self.equalInOut = (in_planes == out_planes)
        #self.convShortcut = (not self.equalInOut) and nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride,padding=0, bias=False) or None

        self.convShortcut = nn.Conv2d(in_planes, self.expansion * out_planes, kernel_size=1, stride=stride, bias=False)
