...
def train_s2m2(base_loader, base_loader_test, model, start_epoch, stop_epoch, params, tmp):
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(base_loader):
            ...
            _, predicted = torch.max(outputs.data, 1)  #计算模型的预测准确率和对输入数据进行旋转变换
            ...
            split_size = int(bs/4)
            for j in indices[0:split_size]:#旋转变换，数据增强的过程
...
if __name__ == '__main__':
    ...
    print(configs.data_dir) #数据目录字典，数据集所在目录
